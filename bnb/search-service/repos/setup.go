...
func Connect() {
	...
	usersCollection = client.Database("bnb-users").Collection("users")	//TODO: not users but not in dockerfile
	opt := options.Index()
	opt.SetUnique(true)
	index := mongo.IndexModel{Keys: bson.M{"email": 1}, Options: opt} //index by email?
